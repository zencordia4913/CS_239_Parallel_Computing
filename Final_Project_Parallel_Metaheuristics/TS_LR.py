...
def tabu_search(num_iterations, num_neighbors, tabu_list_size, num_penalty_mod, num_C_values, num_Solver, num_Learning_rates, X_train_scaled, X_train, y_train, Learning_rates, C_values):
    ...
    for i in range(num_iterations):
        ...
        results = np.empty(len(neighbors), dtype=np.float32)
        d_results = drv.mem_alloc(len(neighbors) * np.float32().itemsize)
        ...
        best_neighbor_index = np.argmax(results)
        # print("Best neighbor index:", best_neighbor_index)
        best_neighbor = neighbors[best_neighbor_index]
        best_neighbor_fitness = results[best_neighbor_index]
        ...
        #print("\n")
        #print(f'After {i} iterations, Best solution: {best_solution}, Best fitness: {best_fitness}')
        #print("\n")
    ...
    return best_solution, best_fitness, cpu_time, gpu_time, function_evaluations
...
for i in range(len(x_values)):
    ...
    best_solution, best_value, cpu_time, gpu_time, function_evaluations = tabu_search(num_iterations, num_neighbors, tabu_list_size, 
                                            num_penalty_mod, num_C_values, num_Solver, num_Learning_rates, 
                                            X_train_scaled, X_train, y_train, Learning_rates, C_values)
    ...
    print("Hyperparameter search method: Parallel Tabu Search")
